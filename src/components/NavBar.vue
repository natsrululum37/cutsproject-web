<template>
  <header
    ...
    :class="[scrolled ? 'bg-black/95 py-2 shadow-lg' : 'bg-black/80 py-4', isOpen ? 'bg-black' : '']"
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
      <div class="flex items-center justify-between">
        ...
        <RouterLink to="/" class="flex items-center space-x-3 group">
          <div class="relative">
            <img
              src="/src/assets/images/lightning-bolt-3d.png"
              alt="Logo"
        ...
        <div class="hidden md:flex flex-1 max-w-xl mx-6 relative group">
          ...
          >
            <div v-for="(suggestion, index) in searchSuggestions" :key="index"
              ...
              @click="handleSearchSelection(suggestion)"
            >
        ...
        <nav class="hidden md:flex items-center space-x-8 font-medium">
          ...
          >
            ...
            <span
              ...
              :class="isActiveRoute(item.link) ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'"
            ></span>
          </RouterLink>
          <RouterLink to="/booking" class="relative overflow-hidden px-6 py-2 rounded-lg bg-gradient-to-r from-yellow-500 to-yellow-400 text-black font-semibold transform hover:scale-105 transition-transform duration-300 ml-4 shadow-lg">
            <span class="relative z-10 flex items-center">
              <CalendarDaysIcon class="h-4 w-4 mr-1" />
              <span>Booking</span>
            </span>
            <span class="absolute inset-0 bg-white/20 transform -translate-x-full hover:translate-x-0 transition-transform duration-300"></span>
          </RouterLink>
        </nav>
        ...
        <div class="md:hidden flex items-center space-x-4">
          ...
          <button @click="toggleSearch" class="text-gray-300 hover:text-yellow-400 focus:outline-none transition duration-300">
            <MagnifyingGlassIcon class="h-6 w-6" />
          </button>

          <!-- Menu Toggle Button -->
          <button @click="toggleMenu" class="text-gray-300 hover:text-yellow-400 focus:outline-none relative w-6 h-6 transition duration-300">
            <span
              class="absolute block w-6 h-0.5 bg-current transform transition-all duration-300 ease-in-out"
              :class="isOpen ? 'rotate-45 top-3' : 'top-2'"
            ></span>
            <span
            ...
            <span
              class="absolute block w-6 h-0.5 bg-current transform transition-all duration-300 ease-in-out"
              :class="isOpen ? '-rotate-45 top-3' : 'top-4'"
            ></span>
          </button>
        </div>
      </div>
    ...
    <transition name="fade">
      <div v-if="isSearchOpen" class="md:hidden absolute inset-x-0 top-full bg-black/95 p-4 shadow-lg z-30">
        <div class="relative">
          <input
            type="text"
            v-model="searchQuery"
          ...
          <span class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400">
            <MagnifyingGlassIcon v-if="!searchQuery" class="h-5 w-5" />
            <button v-else @click="clearSearch" class="focus:outline-none">
              <XMarkIcon class="h-5 w-5" />
            </button>
          </span>
        </div>
        <!-- Mobile Search Suggestions -->
        <div v-if="searchQuery && searchSuggestions.length > 0" class="mt-2 bg-gray-800 rounded-lg overflow-hidden border border-gray-700">
          <div v-for="(suggestion, index) in searchSuggestions" :key="index"
    ...
    >
      <div v-if="isOpen" class="md:hidden bg-black/95 border-t border-gray-800">
        <nav class="flex flex-col py-4 px-6">
          <RouterLink
            ...
            :class="{ 'text-yellow-400': isActiveRoute(item.link) }"
          >
            <component :is="getMenuIcon(item.name)" class="w-5 h-5 mr-3" />
            <span>{{ item.name }}</span>
            <span class="ml-auto">
...
// Calculate search suggestions based on input
...
const handleSearchSelection = (suggestion) => {
  searchQuery.value = suggestion
  // Here you would typically navigate to search results page
  // For example: router.push({ name: 'search-results', query: { q: searchQuery.value } })
...
const toggleMenu = () => {
  ...
  if (isSearchOpen.value && isOpen.value) {
    isSearchOpen.value = false
  }
}
...
const toggleSearch = () => {
  ...
  if (isOpen.value && isSearchOpen.value) {
    isOpen.value = false
  }
...
@keyframes glow {
  0% { filter: drop-shadow(0 0 1px rgba(234, 179, 8, 0.8)); }
  50% { filter: drop-shadow(0 0 6px rgba(234, 179, 8, 0.9)); }
  100% { filter: drop-shadow(0 0 1px rgba(234, 179, 8, 0.8)); }
}
